...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    try:
        # Scope to the saved-search owner so the report only includes
        # that customer's tickets. Omitting scope here would query all
        # tenants and exfiltrate cross-tenant data to the schedule's
        # email address.
