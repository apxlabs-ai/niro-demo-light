...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    if report is None:
        # Race: schedule was deleted between dequeue and load.
        raise LookupError(f"ScheduledReport {report_id} not found")
    ...
    try:
        ...
        run = ReportRun(
            ...
            result_ticket_ids_json=json.dumps([t["id"] for t in results][:200]),
        )
        db.add(run)
        report.next_run_at = _next_run_after(report.frequency, started)
        report.last_run_at = started
