...
from .models import ReportFrequency, ReportRun, Role, SavedSearch, ScheduledReport, User
from .search import FilterError, MAX_SEARCH_RESULT_PAGE_SIZE, execute_search
...
def _saved_search_is_deleted(saved: SavedSearch) -> bool:
...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    if saved is None:
        ...
        db.refresh(run)
        return run
    if _saved_search_is_deleted(saved):
        run = ReportRun(
            scheduled_report_id=report.id,
            ran_at=started,
            success=False,
    ...
    if owner is None:
        ...
        db.refresh(run)
        return run
    if owner.role == Role.agent:
        run = ReportRun(
            scheduled_report_id=report.id,
            ran_at=started,
            success=False,
            error="agent scheduled email reports are disabled",
        )
        db.add(run)
        report.enabled = False
        report.next_run_at = _next_run_after(report.frequency, started)
