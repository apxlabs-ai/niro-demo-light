...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    try:
        # Scope the query to the saved search's owner so the report only
        # contains their tickets. Bypass the cache: scheduled reports
        # need a fresh result set every fire.
