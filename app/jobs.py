...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    try:
        # Scope the search to the saved search's owner so the report only
        # contains that customer's tickets. use_cache=False ensures each
        # scheduled fire gets a fresh result set regardless of TTL.
