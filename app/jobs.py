...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    try:
        # Scope the search to the search owner so each user only receives
        # their own tickets in the emailed report — prevents cross-tenant
        # data exfiltration via the scheduled-report path.
        # Bypass the in-process cache: scheduled reports need a fresh
        # result set every fire to be useful as an audit signal.
