...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    try:
        # Scope the query to the search owner so a customer's scheduled
        # report only contains their own tickets (same restriction as the
        # on-demand run endpoint). use_cache=False because each tick must
        # reflect the current ticket table, not a stale in-process entry.
