...
def is_report_recipient_allowed(owner: User, email: str) -> bool:
    """Agent-owned reports may only go to the agent's own mailbox.
    Customer-owned reports can be sent to the customer-chosen recipient."""
    role = owner.role.value if hasattr(owner.role, "value") else owner.role
    if role != "agent":
        return True
    return email.strip().lower() == owner.email.strip().lower()
...
def run_scheduled_report(report_id: int, db: Session) -> ReportRun:
    ...
    if not is_report_recipient_allowed(owner, report.email):
        run = ReportRun(
            scheduled_report_id=report.id,
            ran_at=started,
            success=False,
