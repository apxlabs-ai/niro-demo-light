...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    Scope must be part of the key so two users with identical filters never
    share a cache slot — a customer-scoped result set must never be served
    to a different customer.
    """
    scope_token = str(scope.id) if scope is not None else "global"
    return hashlib.sha256(f"{scope_token}:{filter_json}".encode()).hexdigest()
