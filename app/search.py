...
def _cache_key(filter_json: str, scope: User | None) -> str:
    """Stable cache key combining filter JSON and caller scope.

    Scope must be part of the key: an agent's global-view result and a
    customer's scoped result for the same filter are different data sets
