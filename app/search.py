...
def _cache_key(filter_json: str, scope: User | None) -> str:
    """Stable cache key derived from filter JSON + caller scope.

    Scope must be part of the key: two users with the same filter get
    different result sets (a customer sees only their tickets; an agent
    sees all). Without scope in the key, one caller's scoped rows would
    be served to every subsequent caller with the same filter.
    """
    scope_tag = f"scope:{scope.id}:{scope.role.value}" if scope is not None else "scope:global"
