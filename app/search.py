...
def _scope_cache_part(scope: User | None) -> str:
    if scope is None:
        return "global"
    return f"{scope.role.value}:{scope.id}"
...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters under the same authorization scope produce the same key.
    The scope partition is part of the security boundary: customer A's
    filtered rows must never be reusable for customer B.
    """
    material = json.dumps(
        {"filter": filter_json, "scope": _scope_cache_part(scope)},
        sort_keys=True,
    )
