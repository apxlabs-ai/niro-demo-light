...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters in the same authorization scope produce the same key — and
    therefore hit the same cache entry. The scope is part of the key so
    customer-scoped rows are never reused across accounts.
    """
    if scope is None:
        scope_key = "global"
