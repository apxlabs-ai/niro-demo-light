...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters produce the same filter component. Customer-scoped searches
    also include the caller identity so one tenant cannot receive rows
    cached for another tenant using the same filter.
    """
    if scope is not None and scope.role == Role.customer:
