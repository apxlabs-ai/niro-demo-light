...
def _cache_key(filter_json: str, scope: User | None = None) -> str:
    """Stable cache key derived from the filter JSON and caller scope.

    Scope is included so Customer A and Customer B get separate cache
    slots even when their filter shapes are identical — without it, the
    first caller's scoped rows would be served to every subsequent caller
    with the same filter (cross-tenant data leak).
    """
