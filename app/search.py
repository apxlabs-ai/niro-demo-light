...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    Scope must be included so two customers with the same filter shape
    don't share a cache entry and receive each other's ticket data.
    """
    scope_part = f"{scope.role.value}:{scope.id}" if scope is not None else "global"
