...
def _cache_key(filter_json: str, scope: User | None = None) -> str:
    # Include the caller's scope so customer A and customer B never share
    # a cache entry for the same filter. Agents and unscoped callers share
    # a "global" namespace — their queries are already unrestricted.
    scope_part = (
        f"customer:{scope.id}"
        if scope is not None and scope.role == Role.customer
        else "global"
