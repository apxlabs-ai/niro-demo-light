...
MAX_SEARCH_RESULT_PAGE_SIZE = 50
...
def _normalize_datetime(value: Any, key: str) -> datetime:
    if isinstance(value, datetime):
        ...
        try:
            ...
            raise FilterError(f"bad {key}: {value!r}") from e
    else:
        raise FilterError(f"bad {key}: {value!r}")
...
def normalize_filter(filter_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in filter_dict.items():
        ...
        elif k in {"created_after", "created_before"}:
            v = _normalize_datetime(v, k)
        out[k] = v
    return out
...
def _scope_key(scope: User | None) -> str:
    if scope is None:
        return "global"
    if scope.role == Role.customer:
        return f"customer:{scope.id}"
    return f"{scope.role.value}:all"


def _paged_cache_key(
    filter_json: str, scope: User | None, limit: int | None, offset: int
) -> str:
    return hashlib.sha256(
        f"{_scope_key(scope)}:{limit}:{offset}:{filter_json}".encode()
    ).hexdigest()
...
def _store_cache_entry(key: str, rows: list[dict[str, Any]]) -> None:
    now = time.time()
    _prune_cache(now)
    if key in _cache:
        _cache.pop(key)
...
def execute_search(
    ...
    *,
    ...
    if use_cache:
        key = _paged_cache_key(canon_json, scope, limit, offset)
        now = time.time()
        _prune_cache(now)
        hit = _cache.get(key)
        if hit is not None:
    ...
    query = _build_query(filter_dict, scope)
    if offset:
        query = query.offset(offset)
    if limit is not None:
        query = query.limit(limit)
    rows = [_ticket_to_dict(t) for t in db.scalars(query).all()]

    if use_cache:
        _store_cache_entry(_paged_cache_key(canon_json, scope, limit, offset), rows)
    return rows
