...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    Scope must be part of the key so two users with identical filters cannot
    share cache entries — otherwise a customer's rows would leak to the next
    caller who runs the same filter within the TTL window.
    """
    scope_tag = f"{scope.id}:{scope.role.value}" if scope is not None else "global"
