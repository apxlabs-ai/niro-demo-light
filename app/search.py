...
def _cache_key(filter_json: str, scope: User | None) -> str:
    """Stable cache key derived from filter JSON AND caller scope.

    Scope is encoded so customer A's rows are never served to customer B.
    Agents see the global view and share a single 'agent' bucket.
    scope=None is the admin/analytics path (also global, separate bucket).
    """
    if scope is None:
        scope_part = "global"
