...
def _cache_key(filter_json: str, scope: User | None = None) -> str:
    ...
    filters produce the same key. But results are scoped per-customer,
    so the key MUST also bind the customer identity — otherwise the
    first customer's result set would be served to any other customer
    running the same filter within the TTL window (cross-tenant leak).
    Non-customer scopes (agent / unscoped analytics) share the global
    entry, which is correct: they all see the same all-tenant rows.
    """
    scope_tag = ""
    if scope is not None and scope.role == Role.customer:
        scope_tag = f":customer:{scope.id}"
    return hashlib.sha256(f"{filter_json}{scope_tag}".encode()).hexdigest()
