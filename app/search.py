...
def _cache_key(filter_json: str, scope: "User | None") -> str:
    ...
    Customers each get their own cache slot so one tenant's results can
    never be returned to another. Agents and analytics callers (scope=None
    or role=agent) share a single global slot — their queries already return
    the full tenant-unscoped result set.
    """
    if scope is not None and scope.role == Role.customer:
        scope_tag = f"|customer:{scope.id}"
    else:
        scope_tag = "|global"
    return hashlib.sha256((filter_json + scope_tag).encode()).hexdigest()
