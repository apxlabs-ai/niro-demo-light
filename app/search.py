...
def _cache_key(filter_json: str, scope: User | None = None) -> str:
    ...
    Scope must be part of the key so that two customers running the same
    filter (e.g. {status: open}) get independent cache entries rather than
    sharing a single entry that leaks one tenant's tickets to another.
    """
    scope_component = str(scope.id) if scope is not None else "global"
    raw = f"{scope_component}:{filter_json}"
