...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    Including scope prevents a cached result for Customer A from being
    served to Customer B when they run the same logical filter. Each
    customer gets their own cache partition; agents share a single
    global partition (they already see all tickets).
    """
    if scope is None:
        scope_tag = "global"
