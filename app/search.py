...
def _cache_key(filter_json: str, scope: "User | None") -> str:
    ...
    Scope must be part of the key because _build_query applies different
    WHERE clauses depending on the caller's role. Without it, a cache
    entry populated by an agent (full-table view) would be served to a
    customer who is only authorized to see their own rows.
    """
    if scope is None:
        scope_part = "scope:none"
    elif scope.role == Role.customer:
        scope_part = f"scope:customer:{scope.id}"
    else:
        scope_part = f"scope:agent:{scope.id}"
    return hashlib.sha256(f"{filter_json}|{scope_part}".encode()).hexdigest()
