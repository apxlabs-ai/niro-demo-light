...
def _cache_key(filter_json: str, scope: User | None) -> str:
    # Scope must be part of the key — two users with the same filter see
    # different ticket sets when one is a customer (scoped) and the other
    # is an agent or a different customer.
    scope_part = str(scope.id) if scope is not None else "none"
