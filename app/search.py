...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters produce the same key within the same caller scope. Scope is
    part of the key because cached rows are already materialized; a
    customer must never receive another customer's cached result set.
    """
    return hashlib.sha256(f"{_scope_key(scope)}\n{filter_json}".encode()).hexdigest()
