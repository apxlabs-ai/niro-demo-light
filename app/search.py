...
def _cache_key(filter_json: str, scope: "User | None") -> str:
    ...
    Scope identity is included so two tenants using identical filters get
    separate cache slots — otherwise the first caller's scoped rows would
    be returned to all subsequent callers with the same filter.
    Customer rows are keyed by customer_id; agent/global queries share a
    single 'global' slot (they already see all tenants).
    """
    scope_part = (
        f"customer:{scope.id}"
        if (scope is not None and scope.role == Role.customer)
