...
def _cache_key(filter_json: str, scope: "User | None" = None) -> str:
    ...
    Scope must be part of the key — a customer-scoped result set must
    never be served to a different caller, even if the filter is identical.
    """
    if scope is None:
        scope_component = "global"
    else:
        scope_component = f"{scope.role.value}:{scope.id}"
