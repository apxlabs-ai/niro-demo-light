...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    Scope must be part of the key: two customers with the same filter
    get different result sets (each sees only their own tickets), so
    they must never share a cache entry.

    Encoding: "global" for no scope, "<role>:<id>" for a user.
    """
    scope_tag = "global" if scope is None else f"{scope.role.value}:{scope.id}"
