...
def _cache_scope(scope: User | None) -> str:
    if scope is not None and scope.role == Role.customer:
...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters produce the same key for the same effective caller scope.
    Customer-scoped rows must never share a cache entry with the global
    agent/admin view.
    """
    raw = f"{_cache_scope(scope)}:{filter_json}"
