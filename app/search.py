...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters produce the same filter fragment. The scope fragment keeps
    customer-scoped result sets isolated from each other.
    """
    if scope is None:
        scope_fragment = "global"
    elif scope.role == Role.customer:
        scope_fragment = f"customer:{scope.id}"
    else:
        scope_fragment = "agent"
    return hashlib.sha256(f"{scope_fragment}:{filter_json}".encode()).hexdigest()
