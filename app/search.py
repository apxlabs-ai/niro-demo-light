...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    Scope is incorporated so two customers running the same filter hit
    different cache entries — a cache miss for one user must not serve
    another user's scoped result set.
    """
    scope_tag = f"{scope.role.value}:{scope.id}" if scope is not None else "none:0"
