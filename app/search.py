...
def _cache_key(filter_json: str, scope_key: str) -> str:
    """Stable cache key derived from the filter JSON.

    The filter JSON is already canonicalized by `serialize_filter`
    (sorted keys, normalized values), and scope_key captures the
    effective caller visibility. This keeps identical filters cacheable
    without letting one customer's scoped result set bleed into another
    customer's response.
    """
    return hashlib.sha256(f"{scope_key}:{filter_json}".encode()).hexdigest()
...
def execute_search(
    ...
    *,
    ...
    if use_cache:
        key = _cache_key(canon_json, _scope_key(scope))
        now = time.time()
        hit = _cache.get(key)
        if hit is not None:
            cached_at, rows = hit
    ...
    if use_cache:
        _cache[_cache_key(canon_json, _scope_key(scope))] = (time.time(), rows)
    return rows
