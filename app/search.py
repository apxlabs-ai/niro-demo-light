...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    filters produce the same key within the same caller scope. Customer
    scope is part of the key because scoped rows are materially different
    result sets even when the filter JSON is identical.
    """
    if scope is None:
        scope_part = "global"
        ...
        scope_part = f"role:{scope.role.value}"
