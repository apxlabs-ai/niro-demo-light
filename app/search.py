...
def _cache_key(filter_json: str, scope: User | None) -> str:
    ...
    The scope is embedded so two callers with different visibility (e.g.,
    Customer A vs Customer B) never share a cache entry — preventing
    cross-tenant data leakage via the cache.

    Scope encoding:
      None            → "global"  (analytics/admin; no user-level filter)
      agent role      → "agent"   (sees all tenants, same global view)
      customer role   → "customer:<user_id>"  (per-user isolation)
    """
    if scope is None:
        scope_tag = "global"
    elif scope.role == Role.agent:
        scope_tag = "agent"
    else:
