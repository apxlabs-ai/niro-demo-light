...
def _load_search_for_owner(
    ...
    """Load a non-deleted saved search owned by the caller."""
    saved = db.get(SavedSearch, search_id)
    if saved is None or saved.deleted:
...
def _load_search_for_write(
    ...
    """Load a saved search for mutation."""
...
def _load_schedule_for_write(
    ...
    """Load a schedule for mutation. The owning saved search controls
    who may change or remove the recurring report."""
    sched = db.get(ScheduledReport, schedule_id)
    if sched is None:
        raise HTTPException(status_code=404, detail="schedule not found")
    saved = db.get(SavedSearch, sched.saved_search_id)
...
def search_stats(
    ...
    total = db.scalar(
        ...
        .where(SavedSearch.deleted.is_(False))
    )
    pinned = db.scalar(
        select(func.count())
        .select_from(SavedSearch)
        .where(SavedSearch.pinned.is_(True), SavedSearch.deleted.is_(False))
    )
    return {
...
def delete_search(
    ...
    saved.deleted = True
    saved.pinned = False
    saved.name = f"deleted-search-{saved.id}"
    saved.filter_json = "{}"
    db.commit()
...
def schedule_report(
    ...
    if user.role == Role.agent:
        raise HTTPException(status_code=403, detail="agent report emails disabled")
...
def disable_schedule(
    ...
    """Disable a schedule while preserving its run history for audit."""
