...
def _load_search_for_write(
    ...
    """Load a saved search for mutation. Agents may inspect customer
    searches through read paths, but only the owner may change or
...
def _load_schedule_for_write(
    ...
    """Load a schedule for destructive changes. Only the owner of the
    backing saved search may disable/delete it."""
    sched = db.get(ScheduledReport, schedule_id)
    if sched is None:
        raise HTTPException(status_code=404, detail="schedule not found")
    saved = db.get(SavedSearch, sched.saved_search_id)
