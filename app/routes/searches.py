...
def _load_search_for_owner(
    ...
    Agents may read any search (for analytics); write operations
    (PATCH, DELETE, schedule creation) are owner-only regardless of role
    to prevent agents from silently mutating or deleting customer data.
    """
    saved = db.get(SavedSearch, search_id)
    if saved is None:
        raise HTTPException(status_code=404, detail="saved search not found")
    if write or user.role != Role.agent:
