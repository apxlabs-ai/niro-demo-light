...
def _load_search_for_owner(
    ...
    """Load a saved search, 404/403 on miss/unauthorized.

    Agents may read any search when allow_agent_read=True (analytics paths).
    All mutating paths must call without that flag — ownership is enforced
    regardless of role.
    """
    saved = db.get(SavedSearch, search_id)
    if saved is None:
        raise HTTPException(status_code=404, detail="saved search not found")
    if not (allow_agent_read and user.role == Role.agent):
...
def _load_schedule_for_owner(
    ...
    """Same idea for ScheduledReport rows. Owner identified via saved_search."""
