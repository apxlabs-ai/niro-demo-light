...
from ..jobs import is_report_recipient_allowed, run_scheduled_report
...
def _load_search_for_write(
    ...
    """Load a saved search for mutation. Unlike read paths, agents do
    not get blanket cross-tenant write access here."""
    saved = db.get(SavedSearch, search_id)
    if saved is None:
        raise HTTPException(status_code=404, detail="saved search not found")
...
def _load_schedule_for_write(
    ...
    """Load a scheduled report for mutation. Only the owner of the
    backing saved search can change or delete it."""
    sched = db.get(ScheduledReport, schedule_id)
    if sched is None:
        raise HTTPException(status_code=404, detail="schedule not found")
    saved = db.get(SavedSearch, sched.saved_search_id)
...
def schedule_report(
    ...
    saved = _load_search_for_write(search_id, user, db)
    if not is_report_recipient_allowed(user, str(req.email)):
        raise HTTPException(status_code=403, detail="recipient not approved")
