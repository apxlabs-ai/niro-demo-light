...
def test_signed_rsa_rejects_replayed_signature(tmp_path, monkeypatch):
    from cryptography.hazmat.primitives import hashes, serialization
    from cryptography.hazmat.primitives.asymmetric import padding, rsa
    ...
    body = b'{"transfer":100,"nonce":"rsa-replay"}'
    sig = base64.b64encode(
        key.sign(body, padding.PKCS1v15(), hashes.SHA256())
    ).decode("ascii")

    with TestClient(app) as client:
        first = client.post("/signed/rsa", content=body, headers={"Signature": sig})
        replay = client.post("/signed/rsa", content=body, headers={"Signature": sig})
