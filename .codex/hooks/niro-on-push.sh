...
#   .codex/hooks/niro-on-push.sh     (Codex CLI)
# Disable by removing the matching hook entry from its agent's
# config file (.claude/settings.json, .github/hooks/niro.json, or
# .codex/hooks.json) or deleting the script copy at that path.
...
#   .codex/hooks.json       "command": "root=\"$(git rev-parse --show-toplevel 2>/dev/null || pwd)\"; bash \"$root/.codex/hooks/niro-on-push.sh\" --agent codex"
#
...
# Copilot, .codex/hooks/ for Codex), so the script must self-detect
# the payload shape.
#
...
if ! command -v jq >/dev/null 2>&1; then
  case "$NIRO_HOOK_AGENT" in
    claude) ;;
    # Codex/Copilot need JSON payload parsing to know whether this
    # hook call is relevant; without jq there is nothing useful or
    # safe to emit, so no-op silently.
    *) exit 0 ;;
  esac
else
...
#
#   - codex: PostToolUse supports the same
#     {hookSpecificOutput.additionalContext} nudge channel. Plain
#     stdout is ignored, so emit JSON.
#
